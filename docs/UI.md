...
**Dropdown (top results):** Up to **8** rows, **ranked** by a simple score (sum of category hits, each category at most once): name **+5**, team **+3**, any matching tag **+2**, notes **+1**. Ties break alphabetically by name. Rows are tuned for scanability: bold name, muted team, and a strongest-match reason when the top match is not name (**`Team: …`**, **`Tag: …`**, or a short **notes** excerpt ~40 chars with ellipsis). Matching substrings in the shown fields are emphasized with a subtle stronger text weight.
